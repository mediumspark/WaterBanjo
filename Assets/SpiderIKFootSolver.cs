...
public class SpiderIKFootSolver : MonoBehaviour
{
    ...
    [SerializeField] SpiderIKFootSolver otherFoot = default;
    ...
    private void Start()
    {
        Pathfinding = GetComponentInParent<AIPath>(); 
        footSpacing = transform.localPosition.x;
        currentPosition = newPosition = oldPosition = transform.position;
        DefaultPosition = transform.localPosition; 
        currentNormal = newNormal = oldNormal = transform.up;
        lerp = 1;
        footdistance = Vector3.Distance(body.position, transform.position);
    }
    ...
    void Update()
    {
        ...
        {
            footdistance = Vector3.Distance(newPosition, info.point);
