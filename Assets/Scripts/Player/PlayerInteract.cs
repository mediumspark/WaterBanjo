...
public class PlayerInteract : MonoBehaviour
{
    ...
    [SerializeField]
    GameObject SRForPlayerInteractable;

    GameObject SRinstance; 

    private void Awake()
    {
        SRinstance = Instantiate(SRForPlayerInteractable); 
    ...
    private void Update()
    {
        ...
        SRinstance.gameObject.SetActive(InteractablesInRange.Length > 0);
        SRinstance.transform.LookAt(Camera.main.transform);

        if(InteractablesInRange.Length > 0)
        {
            SRinstance.transform.position =
                new Vector3(
